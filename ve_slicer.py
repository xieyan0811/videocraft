    ...
    def slice(self, waveform):
        ...
        for i, rms in enumerate(rms_list):
            ...
            if silence_start is not None:
                ...
                is_mid_slice = i - silence_start >= self.min_interval and i - clip_start >= self.min_length
        ...
        if silence_start is not None and len(rms_list) - silence_start >= self.min_interval:
            pos = rms_list[silence_start : silence_start + self.max_sil_kept + 1].argmin() + silence_start
            sil_tags.append((pos, len(rms_list)))

        total_frames = len(rms_list)
        chunks = [[self._apply_slice(waveform, 0, sil_tags[0][0]), 0, sil_tags[0][0] * self.hop_size]] if sil_tags and sil_tags[0][0] > 0 else []
        ...
        if sil_tags and sil_tags[-1][1] < total_frames:
            chunks.append([self._apply_slice(waveform, sil_tags[-1][1], total_frames), sil_tags[-1][1] * self.hop_size, total_frames * self.hop_size])
